...
public class RegisterActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public String getdata(String phoneNumber, String nickname,String vlcode)throws Exception {
        ...
        jsonParam.put("validateCode", vlcode);
        ...
        try {
          ...
          //  showResponse(code,1);
            if(code.equals("0")){
                ...
                appUserId  = JSON.getString("appUserId");
                id  = JSON.getInt("id");
                image  = JSON.getString("image");
                mDBOpenHelper.add(mobile, appUserId,image,id);
    ...
    public void onClick(View view) {
        ...
        final String nickname = registerBinding.etRegisteractivityPassword1.getText().toString().trim();
        final  String getcode =registerBinding.etRegisteractivityPhoneCodes.getText().toString().trim();
        switch (view.getId()) {
                        ...
                        public void onFailure(Call call, IOException e) {
                            RegisterActivity.this.runOnUiThread(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    progressDialog = new ProgressDialog(RegisterActivity.this);
                                    progressDialog.setIcon(R.mipmap.ic_launcher);
                                    progressDialog.setTitle("发送失败");
                                    progressDialog.setMessage("网络错误，请稍后再试!");
                                    progressDialog.setIndeterminate(true);
                                    progressDialog.setCancelable(true);
                                    progressDialog.show();
                                }
            ...
            case R.id.bt_registeractivity_register:
               // showResponse("有东西吗？"+getcode+nickname+phone,1);
                try {
                    formdata =  getdata(phone,nickname,getcode);
                } catch (Exception e) {
                    e.printStackTrace();
                }
                if (!TextUtils.isEmpty(phone) &&!TextUtils.isEmpty(nickname)&& !TextUtils.isEmpty(getcode)&&phone.length()==11) {
                   new Thread() {
                        public void run() {
